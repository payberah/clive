...
public class ScenarioChurnReal extends Scenario {
	private static SimulationScenario scenario = new SimulationScenario() {{
		...
		StochasticProcess churn = new StochasticProcess() {{
			eventInterArrivalTime(exponential(200));
			raise(1000, Operations.msJoin(Configuration.PARENT_SIZE), distro, uniform(13));
			raise(1000, Operations.msJoin(Configuration.PARENT_SIZE), constant(BigInteger.ZERO), uniform(13));
//			raise(1000, Operations.msJoin(Configuration.PARENT_SIZE), constant(BigInteger.ZERO), uniform(13));
		...
		churn.startAfterTerminationOf(20000, class5);
	}};
	
//-------------------------------------------------------------------
	public ScenarioChurnReal() {
		super(scenario);
	} 
}
